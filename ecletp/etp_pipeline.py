...
etp_pipeline.py
----------------
ETP connection + main pipeline as a reusable module (no CLI here).
- Discovers RESQML IjkGridRepresentation over ETP (FETPAPI)
- Builds FESAPI repository
- Calls conversion/writers from resqml2grdecl.py
...
def open_etp_session(ws_url: str, authorization: str, partition: str, timeout_s: float = 15.0):
    ...
    t = _th.Thread(target=session.run, name="fetpapi-run", daemon=True)
    t.start()
...
def _filter_grids_by_uuid(grids: List[EtpResource], uuid_list: List[str]) -> List[EtpResource]:
    ...
    return kept
# --------------------------------- Main pipeline --------------------------------- #
def process_all(cfg, out_dir: str, filter_uuids: List[str], export_egrid: bool, local_xy_mode: bool):
    import xtgeo  # only needed if export_egrid
